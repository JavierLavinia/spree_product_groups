    ...
    module Search
      class ProductGroupBase
        ...
        def retrieve_products
          ...
          @products = @products_scope.includes(:master).page(curr_page).per(per_page).group("spree_products.id")
        end
        ...
        protected
          ...
          def prepare(params)
            ...
            if !params[:order_by_price].blank?
              ...
              @cached_product_group = @cached_product_group.from_route(params[:product_group_query].split('/')) if @cached_product_group && params[:product_group_query]
